				...
				<div>
					...
					<?php if(user('id') == $row['id']):?>
						...
						<a href="gesundheit.php">
							<button class="mx-auto m-1 btn-sm btn btn-info text-white">Add Gesundheitsinfos</button>
						</a><br>
							<a href="zeigen1.php">
							<button class="mx-auto m-1 btn-sm btn btn-info text-white">Show Gesundheitsinfos</button>
						</a>
					<?php endif;?>
				</div>
			</div>
				...
				<table class="table table-striped">
					...
					<tr><th><i class="bi bi-envelope"></i> Email</th><td><?=esc($row['email'])?></td></tr>
							 ...
							 <?php
								...
								$sql1="SELECT BMI  FROM `gesundheitsinfos`INNER JOIN users ON gesundheitsinfos.Mitarbeiter_ID=users.id WHERE users.id=$id GROUP by datum DESC  LIMIT 1";
								$query1=mysqli_query($verbindung,$sql1);
								while($ergebnis1=mysqli_fetch_assoc($query1))
								{
								$BMI=$ergebnis1["BMI"];
								echo "<tr><th><i class=\"bi bi-calculator-fill\"></i> BMI</th><td>".$BMI."</td></tr>";
								}
								?>
